...
# Freq
ggplot(patientInfo, aes(x = Freq)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black", alpha = 0.7) +
  ggtitle("Frequency of Visit Histogram")

# bloodp
ggplot(patientInfo, aes(x = bloodp)) +
  geom_histogram(binwidth = 1, fill = "green", color = "black", alpha = 0.7) +
  ggtitle("Blood Pressure Histogram")

# first
ggplot(patientInfo, aes(x = first)) +
  geom_histogram(binwidth = 1, fill = "red", color = "black", alpha = 0.7) +
  ggtitle("First Decision by General Doctor Histogram")

# second
ggplot(patientInfo, aes(x = second)) +
  geom_histogram(binwidth = 1, fill = "purple", color = "black", alpha = 0.7) +
  ggtitle("Second Decision by External Doctor Histogram")

# finaldecision
ggplot(patientInfo, aes(x = finaldecision)) +
  geom_bar() +
  ggtitle("Final Decision by Head of Emergency Unit Histogram") +
  xlab("Final Decision") +
  ylab("Frequency")
